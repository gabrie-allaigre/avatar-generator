...
public class TriangleElementRegistry extends ElementRegistry {
	...
	private Path buildImage(IAvatarInfo avatarInfo) {
		...
		Color color = colors
				.get(avatarInfo.getRandom().nextInt(colors.size()));
		...
		return AvatarUtils.saveImageInTemp(AvatarUtils.planImage(dst, size - (int) ss, size - (int) ss));
	}
